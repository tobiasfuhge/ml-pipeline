...
def load_best_model_and_data(training_run_id, preprocessing_run_id):
    ...
    if not child_runs:
        raise RuntimeError("No child runs found")
    ...
    val_df = pd.read_csv(os.path.join(tmp, "processed_data", "val.csv"))
...
def main():
    ...
    with mlflow.start_run(run_name="evaluation") as run:
        ...
        gate_sanity = np.isin(y_pred, valid_labels).all()

        promote = all([gate_f1, gate_precision, gate_drift, gate_sanity])
        ...
        # ✅ RETURN PROMOTION DECISION TO AIRFLOW
        write_xcom({
